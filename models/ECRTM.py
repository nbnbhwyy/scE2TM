...
class ECRTM(nn.Module):
    def __init__(self, args):
        ...
        self.TAC1 = ClusterHead(in_dim=512, num_clusters=100).cuda()
    ...
    def get_embedding(self):
        return self.topic_embeddings,self.word_embeddings
    ...
    def forward(self, text, image, neigh_text, neigh_image, flag = 0, z2 = None):
        z, loss_KL = self.encode(text)
        theta = F.softmax(z, dim=1)
        beta = self.get_beta()
        recon = F.softmax(self.decoder_bn(torch.matmul(theta, beta)), dim=-1)
        recon_loss = -(text * recon.log()).sum(axis=1).mean()
        loss_TM = recon_loss + 1*loss_KL
        loss_ECR = self.get_loss_ECR()


        logit_image = self.TAC1(image)
        neigh_logit_image = self.TAC1(neigh_image)
        neigh_logit_text = self.encode(neigh_text, flag = 0)
        logit_text = self.encode(text, flag = 0)
        loss_distill = self.distill_loss(logit_image, neigh_logit_text) + self.distill_loss(logit_text, neigh_logit_image)
        loss_consist = consistency_loss(logit_text, logit_image)
        loss_entropy = entropy(logit_text) + entropy(logit_image)
        loss_TAC = loss_distill + 1*loss_consist - 5* loss_entropy 
        loss = loss_TM  + loss_ECR + 1*loss_TAC
